...
export default class PasskitSDK {
  ...
  doQuery(http_method, endpoint, req_data, http_options_param, callback) {
    ...
    //console.log(this.get_default_options().url + endpoint);
  ...
  createPassAsync(templateName, userDefinedId, dynamicData, recoveryEmail) {
    const _this = this;
  ...
  getPassAsync(passId) {
    const _this = this;
    return new Bluebird((resolve, reject)=> {
      _this.getPass(passId,(err, result) => {
        if (err)
          reject(err);
        else
          resolve(result);
